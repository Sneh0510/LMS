            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img src={assets.dropdown_icon} onClick={() => handleChapter('toggle', chapter.chapterId)}  width={14} alt="" className={`mr-2 cursor-pointer transition-transform duration-300 transform ${chapter.collapsed ? '-rotate-90' : 'rotate-0'}`}/>
                  <span className='font-semibold'>{chapterIndex + 1} {chapter.chapterTitle}</span>
                </div>
                <span className='text-gray-500'>{chapter.chapterContent.length} Lectures</span>
                <img src={assets.cross_icon} alt="Remove Chapter" onClick={() => handleChapter('remove', chapter.chapterId)} className='cursor-pointer'/>
              </div>
              {!chapter.collapsed && (
                <div className='p-4'>
